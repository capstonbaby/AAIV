...
public class CameraStreamFragment extends Fragment
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        mPreviewImageView = (ImageView) view.findViewById(R.id.iv_preview);
    ...
    public void CreateImageFile() {
        ...
        String prepend = "/AAIV_" + timestamp + ".jpg";
        mFile = new File(getActivity().getExternalFilesDir(null), prepend);
    }
    ...
    private class FaceIdentify extends AsyncTask<UUID, Void, IdentifyResult[]> {
            ...
            if (identifyResults != null) {
                for (IdentifyResult result : identifyResults) {
                    if (result.candidates.size() > 0) {
                        new PersonInfo().execute(result.candidates.get(0).personId);
                    } else {
                        mTvResult.post(new Runnable() {
                            @Override
                            public void run() {
    ...
    public class PersonInfo extends AsyncTask<UUID, Void, Person> {
        ...
        protected void onPostExecute(final Person person) {
            if (person != null) {
                Log.d("identify", person.name);
                mTvResult.post(new Runnable() {
                    @Override
                    public void run() {
                        mTvResult.setText(person.name);
                    }
                });
                Speak(person.name);
                //takePicture();
        ...
        mTextToSpeech = new TextToSpeech(getContext(), new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                if (status != TextToSpeech.ERROR) {
                    ...
                    Toast.makeText(getContext(), "Setup Speech Failed", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
